...
impl<T> Arena<T> {
    ...
    #[cfg(feature = "uuid")]
    pub fn match_id(&self, id: &Key<T>) -> bool {
        id.uuid == self.uuid
    }
    ...
    ///
    /// ```
    pub fn get2_mut(&mut self, a: Key<T>, b: Key<T>) -> (Option<&mut T>, Option<&mut T>) {
        #[cfg(feature = "uuid")]
    ...
    /// ```
    #[inline]
    pub fn contains(&self, id: Key<T>) -> bool {
        #[cfg(feature = "uuid")]
        if !self.match_id(&id) {
            return false;
        }
    ...
    /// ```
    #[inline]
    pub fn id_at(&self, index: usize) -> Option<Key<T>> {
        if index >= self.len() {
            return None;
        }
        let idx = self.slots.get(index)?.value_slot;
        match &self.slots[idx].state {
            State::Used { uid, value } if *value == index => Some(Key::<T> {
                #[cfg(feature = "uuid")]
                uuid: self.uuid,
                uid: *uid,
                idx,
    ...
    /// ```
    #[inline]
    pub fn index_of(&self, id: Key<T>) -> Option<usize> {
        #[cfg(feature = "uuid")]
        if !self.match_id(&id) {
            return None;
        }
    ...
    /// ```
    pub fn insert_with<F>(&mut self, create: F) -> Key<T>
    where
        F: FnOnce(Key<T>) -> T,
    {
    ...
    /// ```
    #[inline]
    pub fn swap_positions(&mut self, i: Key<T>, j: Key<T>) -> bool {
        #[cfg(feature = "uuid")]
